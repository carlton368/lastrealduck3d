...
float ComputeDistanceXYZ(float3 positionWS, float startDistance, float distanceDensity)
{
	...
	float falloff = length(_WorldSpaceCameraPos.xyz - positionWS.xyz);

	float3 viewDir = normalize(positionWS - _WorldSpaceCameraPos);
	half horizon = 1-saturate(viewDir.y);

	//Attenuate the fog by the vertical viewing angle. This yield correct results when looking up
	horizon = smoothstep(0.2, 1, horizon);
	falloff *= horizon;
	
	//Start distance
	falloff -= _ProjectionParams.y + startDistance;
	falloff *= distanceDensity;
	
	return (1-(exp(-falloff)));
}
...
float4 GetUnderwaterFogColor(float4 shallow, float4 deep, float distanceDensity, float heightDensity)
{
	...
	waterColor.rgb = lerp(shallow.rgb, deep.rgb, saturate(distanceDensity * 2.0)) * _UnderwaterFogBrightness;
